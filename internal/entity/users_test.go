...
func testUserToManyMinistryLeaders(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadMinistryLeaders(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpMinistryLeaders(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `FirstName`: `character varying`, `LastName`: `character varying`, `Email`: `character varying`, `HashedPassword`: `character varying`, `Birthday`: `date`, `Phone`: `character varying`, `OutreachID`: `uuid`, `CellLeaderID`: `uuid`, `CreatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
