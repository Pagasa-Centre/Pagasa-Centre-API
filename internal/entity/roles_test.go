...
func testRoleToManyUsers(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"user_roles\" (\"role_id\", \"user_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"user_roles\" (\"role_id\", \"user_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	roleDBTypes = map[string]string{`ID`: `integer`, `RoleName`: `character varying`}
	_           = bytes.MinRead
)
