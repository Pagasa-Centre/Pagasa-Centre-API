...
func testEventToManyEventDays(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, eventDayDBTypes, false, eventDayColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, eventDayDBTypes, false, eventDayColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.EventDays().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := EventSlice{&a}
	if err = a.L.LoadEventDays(ctx, tx, false, (*[]*Event)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.EventDays); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.EventDays = nil
	if err = a.L.LoadEventDays(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.EventDays); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testEventToManyAddOpEventDays(t *testing.T) {
	var err error
	...
	foreigners := []*EventDay{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, eventDayDBTypes, false, strmangle.SetComplement(eventDayPrimaryKeyColumns, eventDayColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*EventDay{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddEventDays(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.EventDays[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.EventDays[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.EventDays().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	eventDBTypes = map[string]string{`ID`: `uuid`, `Title`: `text`, `Description`: `text`, `AdditionalInformation`: `text`, `Location`: `text`, `RegistrationLink`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
